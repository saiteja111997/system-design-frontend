...
export function createMouseDownHandler(
  ...
  activeTool: Tool,
  setDrawingState: (
    state: DrawingState | ((prev: DrawingState) => DrawingState)
  ) => void,
    ...
    if (activeTool === "text") {
      ...
      timeoutId = setTimeout(() => {
        ...
        try {
          ...
          console.warn(
            "[AnnotationLayer] Failed to enter text edit mode:",
            error
          );
          cleanup();
        }
...
export function createMouseUpHandler(
  ...
  return () => {
    ...
    if (drawingState.currentShape) {
      ...
      const shapeWithType = drawingState.currentShape as unknown as {
        arrowType?: string;
      };
...
function addArrowhead(canvas: FabricCanvas, lineShape: FabricObject): void {
  try {
    ...
    if (fabricNs.Line) {
      const arrowLine1 = new fabricNs.Line(
        [x2, y2, arrowPoint1X, arrowPoint1Y],
        {
      ...
      const arrowLine2 = new fabricNs.Line(
        [x2, y2, arrowPoint2X, arrowPoint2Y],
        {
          stroke: line.stroke || "#000",
          strokeWidth: line.strokeWidth || 2,
          selectable: false,
      ...
      (
        lineShape as unknown as { arrowheadLines?: FabricObject[] }
      ).arrowheadLines = [arrowLine1, arrowLine2];
    }
  } catch (e) {
