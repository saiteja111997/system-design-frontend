...
const fabricNsRaw = (fabricModule.fabric || fabricModule) as
  | Partial<FabricNamespace>
  | undefined;
function ensureFabric<K extends keyof FabricNamespace>(
  feature: K
): FabricNamespace[K] {
 ...
 */
export function getDefaultDrawingConfig(
  theme?: "dark" | "light"
): DrawingToolConfig {
...
export function configureBrush(
  ...
  try {
    if (!canvas.freeDrawingBrush) {
      ...
      const PencilBrushCtor = (
        fabricNsRaw as unknown as {
          PencilBrush?: new (c: FabricCanvas) => {
            width: number;
            color: string;
          };
        }
      ...
      if (PencilBrushCtor) {
        ...
        ).freeDrawingBrush = new PencilBrushCtor(canvas);
      }
    }
    if (canvas.freeDrawingBrush) {
  ...
  try {
    ...
    canvas.getObjects().forEach((obj) => {
      ...
      if (t && ["rect", "circle", "path", "line"].includes(t)) {
        ...
        (
          obj as unknown as { set: (props: Record<string, unknown>) => void }
        ).set?.({ stroke: cfg.strokeColor });
      } else if (t && ["i-text", "text"].includes(t)) {
        // For text objects, update fill instead of stroke
        (
          obj as unknown as { set: (props: Record<string, unknown>) => void }
        ).set?.({ fill: cfg.strokeColor });
      }
    });
...
export function createArrow(
  ...
  (
    line as unknown as { arrowType?: string; arrowHeadSize?: number }
  ).arrowType = "arrow";
  (
    line as unknown as { arrowType?: string; arrowHeadSize?: number }
  ).arrowHeadSize = 10;
...
export function updateCanvasMode(
  ...
  const activeObject =
    activeObjects && activeObjects.length > 0 ? activeObjects[0] : null;
  ...
  const normalizedTool:
    ...
    if (["freedraw", "freehand", "draw", "brush"].includes(activeTool))
      return "freehand";
  ...
  if (normalizedTool === "freehand") {
    ...
    canvas.defaultCursor =
      normalizedTool === "select" ? "default" : "crosshair";
