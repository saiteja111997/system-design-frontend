...
type AnnotationCanvas =
  import("../../../../utils/annotationUtils").FabricCanvas;
...
export function useAnnotationInternal(
  ...
  const { historyManagerRef, saveToHistory, saveToHistoryImmediate } =
    useHistoryManager(canvas, blockHistorySavesRef);
  ...
  const handlers = useMemo<AnnotationLayerHandle>(
    () => ({
      snapshot: () =>
        canvas ? serializeCanvas(canvas as AnnotationCanvas) : null,
      load: async (json) => {
        if (canvas && json) {
          try {
            ...
            console.error(
              "[AnnotationLayer] Failed to load canvas state:",
              error
            );
          }
        }
        ...
        if (data.currentIndex >= 0 && data.currentIndex < data.history.length) {
          ...
          if (canvas && currentState) {
            restoreCanvas(canvas as AnnotationCanvas, currentState)
              ...
              .catch((error) => {
                console.error(
                  "[AnnotationLayer] Failed to restore canvas after history import:",
                  error
                );
              });
          }
  ...
  const handleMouseMove = useCallback(
    (e?: FabricEvent) => {
      if (!canvas) return;
      createMouseMoveHandler(canvas, activeTool, drawingState)(e);
    },
    [canvas, activeTool, drawingState]
  );
  ...
  }, [canvas, drawingState, setDrawingState, saveToHistory, onFinish]);
  ...
  useEffect(() => {
    ...
    const relevant = (t?: string) =>
      !!t &&
      ["rect", "circle", "path", "line", "i-text", "text", "textbox"].includes(
        t
      );
  ...
  useEffect(() => {
    ...
    ).on("path:created", pathCreated);
    return () => {
      (
        canvas as unknown as {
          off: (event: string, handler: (e?: unknown) => void) => void;
        }
      ).off("path:created", pathCreated);
    };
  }, [canvas, saveToHistoryImmediate]);
  ...
  const handleKeyDown = useCallback(
    (e: KeyboardEvent) => {
      if (!canvas) return;
      createKeyDownHandler(canvas, activeTool, handlers.undo, handlers.redo)(e);
    },
    [canvas, activeTool, handlers.undo, handlers.redo]
  );
  useEffect(() => {
    ...
    return () => document.removeEventListener("keydown", wrap);
  }, [handleKeyDown]);
  ...
  useEffect(() => {
    try {
      ...
      setInitError(
        e instanceof Error ? e.message : "Failed to initialize canvas"
      );
    }
    return cleanup;
  }, [initializeCanvas, cleanup]);
  ...
  useEffect(() => {
    ...
    const handleDeselection = () => {
      ...
      objects.forEach((obj) => {
        ...
        if (
          objWithType.type === "textbox" ||
          objWithType.type === "i-text" ||
          objWithType.type === "text"
        ) {
          objWithType.set?.({
            hasBorders: false,
            hasControls: false,
  ...
  useEffect(() => {
    if (!canvas || !isReady || initError || initialHistorySavedRef.current)
      return;
  ...
  const retry = useCallback(() => {
    ...
    try {
      ...
      setInitError(e instanceof Error ? e.message : "Retry failed");
    }
  }, [initializeCanvas]);
