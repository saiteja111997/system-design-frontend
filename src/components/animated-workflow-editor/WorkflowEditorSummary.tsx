...
interface WorkflowEditorSummaryProps {
  value: number;
  onChange: (value: number) => void;
}

export const WorkflowEditorSummary: React.FC<
  WorkflowEditorSummaryProps
> = ({ value, onChange }) => {
      ...
      <div className="p-4 grid grid-cols-2 gap-4">
        ...
        <div className="bg-slate-800/50 rounded-lg p-3 border border-slate-600">
          <div className="flex items-center space-x-2 mb-1">
            <div className={`w-2 h-2 rounded-full ${rpsColorClass} animate-pulse`}></div>
        ...
        <div className="bg-slate-800/50 rounded-lg p-3 border border-slate-600">
          ...
          <div className={`text-xl font-bold font-mono ${getLoadColor(serverLoad).split(' ')[0]}`}>
            {serverLoad}%
          </div>
        </div>
        ...
        <div className="bg-slate-800/50 rounded-lg p-3 border border-slate-600">
          <div className="flex items-center space-x-2 mb-1">
            <div className={`w-4 h-4 rounded-full border-2 ${iconClass} flex items-center justify-center`}>
              <div className={`w-2 h-2 rounded-full ${iconClass.replace('text-', 'bg-')}`}></div>
            </div>
            <span className="text-xs font-medium text-slate-400 uppercase tracking-wide">
              STATUS
            </span>
          </div>
          <div className={`text-lg font-bold ${statusClass}`}>
            {status}
          </div>
        </div>
